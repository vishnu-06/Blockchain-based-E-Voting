...
from totp import create, verify
...
from .forms import (
    ...
    ResendActivationCodeForm, ResendActivationCodeViaEmailForm, ChangeProfileForm, ChangeEmailForm, VerifyDetailsForm
)
...
class LogInView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        act = Activation.objects.filter(user_id = form.user_cache)[0]
        otp_is_valid = verify.validate_otp(form.cleaned_data['two_fa_totp'], act.totp_id)
        ...
        if(otp_is_valid):
            ...
            messages.warning(request, _('OTP is Incorrect.'))
            logout(request)
...
class SignUpView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        if settings.ENABLE_USER_ACTIVATION:
            ...
            act.code = code
            act.user = user
            act.totp_id = create.create_user(user.id,user.username)
            act.email = user.email
            key = ECC.generate(curve='P-256')
            act.public_key_ecc = key.public_key().export_key(format = "OpenSSH")
            act.private_key_ecc = key.export_key(format = "PEM")
            act.save()
            ...
            messages.success(
                request, _('You are successfully signed up.'))
        else:
            raw_password = form.cleaned_data['password1']
...
class VerifyAccount(LoginRequiredMixin, FormView):
    template_name = 'accounts/verify_details.html'
    form_class = VerifyDetailsForm

    def form_valid(self, form):
        user_activation = Activation.objects.filter(user_id = self.request.user.id)
        if(len(user_activation)): 
            user_activation = user_activation[0]
            user_activation.aadhaar_id = form.cleaned_data['aadhaar_id']
            user_activation.voter_id = form.cleaned_data['voter_id']
            user_activation.phone_number = form.cleaned_data['phone_number']
            user_activation.address = form.cleaned_data['address']
            user_activation.voting_allowed = "PEN"
            # Create a user record
