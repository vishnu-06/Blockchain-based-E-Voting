...
class SignInViaEmailOrUsernameForm(SignIn):
    ...
    two_fa_totp = forms.CharField(label = _('Enter 2FA OTP'))
...
class VerifyDetailsForm(forms.Form):
    address = forms.CharField(label=_('Address'), max_length=300, required=True)
    phone_number = forms.CharField(label=_('Phone Number'), max_length=10, required=True)
    aadhaar_id = forms.CharField(label=_('Aadhaar Number'), max_length=16, required=True)
    voter_id = forms.CharField(label=_('Voter ID Number'), max_length=16, required=True)
